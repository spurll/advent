...
and, as luck would have it, good ol' Aunt Sue got you a My First Crime Scene Analysis
...
compounds in a given sample, as well as how many distinct kinds of those compounds there
are. According to the instructions, these are what the MFCSAM can detect:
...
your eye. Apparently, it has an outdated retroencabulator, and so the output from the
...
pomeranians and goldfish readings indicate that there are fewer than that many (due to the
...
found(sue) = all(Bool[in(pair, target) for pair in sue])

function found_2(sue)
    for (property, count) in sue
        if in(property, ["cats", "trees"])
            (count <= target[property]) && return false
        elseif in(property, ["pomeranians", "goldfish"])
            (count >= target[property]) && return false
        else
            (count != target[property]) && return false
        end
...
for line in eachline(f)
    number, properties = match(r"^Sue (\d+): (.+)$", line).captures
    aunt = Dict([
        item.captures[1] => parse(item.captures[2])
        for item in eachmatch(r"(\w+): (\d+)", properties)
    ])
    found(aunt) && println("Part 1: $number")
    found_2(aunt) && println("Part 2: $number")
end
close(f)
