...
The Elves decide they don't want to visit an infinite number of houses. Instead, each Elf
will stop after delivering presents to 50 houses. To make up for it, they decide to deliver
...
#function factors(n) = [Int[i for i in filter(x -> n % x == 0, 1:(n / 2))], n]
...
delivered_2(n) = filter(x -> n <= x * 50, factors(n))

presents_1(n) = sum(factors(n)) * 10
presents_2(n) = sum(delivered_2(n)) * 11
