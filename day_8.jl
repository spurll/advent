...
(2 + 5 + 10 + 6 = 23) minus the total number of characters in memory for string values (0 +
...
original code representation (23, just like in the first part of this puzzle) is 42 - 23 =
19.
=#

literal_minus_memory = 0
encoded_minus_literal = 0

f = open("day_8.txt")
for line in eachline(f)
    literal_minus_memory += length(strip(line)) - length(eval(parse("b$line")))
    encoded_minus_literal += length(repr(strip(line))) - length(strip(line))
end
close(f)

println(literal_minus_memory)
println(encoded_minus_literal)
