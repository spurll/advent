...
Today, you set out on the task of perfecting your milk-dunking cookie recipe. All you have
to do is find the right balance of ingredients.
...
total score of 62842880, which happens to be the best score possible given these
ingredients. If any properties had produced a negative total, it would have instead become
...
has exactly 500 calories per cookie (so they can use it as a meal replacement). Keep the
rest of your award-winning process the same (100 teaspoons, same ingredients, same scoring
system).
...
ingredients = Dict{AbstractString, Dict}()
p = r"(\w+): (\w+) ([\d-]+), (\w+) ([\d-]+), (\w+) ([\d-]+), (\w+) ([\d-]+), (\w+) ([\d-]+)"

f = open("day_15.txt")
for line in eachline(f)
    m = match(p, line).captures
    ingredients[m[1]] = Dict{AbstractString, Int}([m[i] => parse(m[i + 1]) for i in 2:2:10])
end
close(f)
...
function score(recipe)
    return reduce(*, [
        max(0, sum([
            recipe[i] * ingredients[i][property] for i in keys(recipe)
        ]))
        for property in collect(filter(x -> x != "calories", keys(first(ingredients)[2])))
    ])
end

function calories(recipe)
    return sum([recipe[i] * ingredients[i]["calories"] for i in keys(recipe)])
end
...
for combinations in partitions(100, length(ingredients))
    for distribution in permutations(combinations)
        recipe = Dict(
            [name => distribution[i] for (i, name) in enumerate(keys(ingredients))]
        )
        ...
        if (calories(recipe) == 500) && (s > best_500)
            best_500 = s
        end
    end
end
