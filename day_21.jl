...
boss = (
    ...
    parse(match(r"Armor: (\d+)", input).captures...)
)
...
r_combos = unique(combinations(rings, 2))

options = []
for w in weapons
    for a in armors
        for r in r_combos
            push!(options, ([w...] + [a...] + [r[1]...] + [r[2]...]...))
        end
    end
end
sort!(options)
...
for o in options
    if win(Character(100, o[2], o[3]), Character(boss...))
        println("Paid $(o[1]) gold to win.")
        break
    end
end
