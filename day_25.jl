...
you to enter a code from the instruction manual. Apparently, it refuses to run unless you
...
value, 20151125. Multiply it by 252533 to get 5088824049625. Then, divide that by 33554393,
...
index(r, c) = index(r + (c - 1)) + c - 1

index(r) = sum(1:(r - 1)) + 1

code(r, c) = code(index(r, c))

# Brute Force (Stack Overflow)
# code(n) = (n == 1) ? 20151125 : (code(n - 1) * 252533) % 33554393
...
println(code(
    map(parse, match(r"Enter the code at row (\d+), column (\d+).", input).captures)...
))
