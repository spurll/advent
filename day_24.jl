...
there's one small problem: he can't get the sleigh to balance. If it isn't balanced, he
...
package has to fit. The first group goes in the passenger compartment of the sleigh, and the
second and third go in containers on either side. Only when all three groups weigh exactly
...
doesn't matter how many packages are in either of the other two groups, so long as all of
the groups weigh the same.

Furthermore, Santa tells you, if there are multiple ways to arrange the packages such that
the fewest possible are in the first group, you need to choose the way where the first group
has the smallest quantum entanglement to reduce the chance of any "complications". The
quantum entanglement of a group of packages is the product of their weights, that is, the
value you get when you multiply their weights together. Only consider quantum entanglement
...
this situation, the unique first groups, their quantum entanglements, and a way to divide
...
Given the example packages above, this would be the new unique first groups, their quantum
...
f = open("day_24.txt")
packages = map(parse, readlines(f))
close(f)

group_weight = div(sum(packages), num_groups)

# Not used, although writing it was instructive.
function combos(a, target_sum)
    c = []
    for n = 1:length(a)
        append!(c, collect(filter(x -> sum(x) == target_sum, combinations(a, n))))
    end
    return c
end

# Returns true if the group can be broken down into N groups that each sum to the target
# weight. Note that if we can make N - 1 valid groups then we're happy, because (unless
# the packages have been changed) the last one must also work!
function combos_exist(a, groups_to_find)
    for n = 1:length(a)
        !isempty(filter(
            function (group)
                (sum(group) == group_weight) && (
                    (groups_to_find <= 2) ||
                    combos_exist(setdiff(a, group), groups_to_find - 1)
                )
            end,
            combinations(a, n))
        ) && return true
...
# Determine possible configurations, picking the one that has the smallest group.
for group_size in 1:length(packages)
    groups = combinations(packages, group_size)
    groups = filter(group -> sum(group) == group_weight, groups)
    groups = filter(group -> combos_exist(setdiff(packages, group), num_groups - 1), groups)

    # Among the groups, pick the one with the smallest product.
    smallest_product = Inf
    for group in groups
        product = reduce(*, group)
        if product < smallest_product
            smallest_product = product
        end
    end

    if smallest_product < Inf
        println(smallest_product)
        break
    end
end
