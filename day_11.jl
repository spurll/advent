...
of coming up with a password based on the previous one. Corporate policy dictates that
passwords must be exactly eight lowercase letters (for security reasons), so he finds his
new password by incrementing his old password string repeatedly until it is valid.
...
function increment(password)
    ...
    if password[end] < 'z'
        return "$(password[1:(end - 1)])$(password[end] + 1)"
    else
        return "$(increment(password[1:(end - 1)]))a"
    end
end

function valid(password)
    test = "abc"
    sequence = contains(password, test)
    while !sequence && (test < "xyz")
        test = string(broadcast(+, collect(test), 1)...)
        sequence = contains(password, test)
    end

    banned = ismatch(r"[iol]", password)
    doubles = ismatch(r"([a-z])\1.*([a-z])\2"i, password)

    return sequence && doubles && !banned
end
