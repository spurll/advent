...
benefactor. It comes with instructions and an example program, but the computer itself seems
to be malfunctioning. She's curious what the program does, and would like you to help her
run it.
...
backward, respectively). For example, jmp +1 would simply continue with the next
...
computer. Definitely not to distract you, what is the value in register b after the program
...
instructions = []
state = Dict{Symbol, Int}(:a => 0, :b => 0, :context => 1)

f = open("day_23.txt")
for line in eachline(f)
    push!(instructions, map(
        x -> is(x, nothing) ? nothing : parse(x),
        match(r"(\w+) ([+\-\w\d]+)(?:, ([+\-\d]+))?", line).captures
    ))
end
close(f)

while 1 <= state[:context] <= length(instructions)
    instruction = instructions[state[:context]]
    eval(instruction[1])(instruction[2], instruction[3])
end

println(state[:b])

state = Dict{Symbol, Int}(:a => 1, :b => 0, :context => 1)
while 1 <= state[:context] <= length(instructions)
    instruction = instructions[state[:context]]
    eval(instruction[1])(instruction[2], instruction[3])
end
