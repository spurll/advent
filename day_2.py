...
for box in instructions:
    dims = [int(dim) for dim in box.split('x')]

    side_area = [dims[i] * dims[(i + 1) % 3] for i in range(3)]
    side_perimeter = [2 * dims[i] + 2 * dims[(i + 1) % 3] for i in range(3)]

    total_area += sum([2 * side for side in side_area])
    extra_area += min(side_area)

    ribbon += min(side_perimeter)
    bow += dims[0] * dims[1] * dims[2]

print(total_area + extra_area)
print(ribbon + bow)
