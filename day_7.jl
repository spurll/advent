...
specific value. Each wire can only get a signal from one source, but can provide its signal
...
# This should clearly be a recursive solution, instead.
...
for line in eachline(f)
    # Using uppercase is a cheap hack to make sure we don't use reserved words.
    input, output = match(r"(.+) -> (.+)", uppercase(line)).captures
    for operator in map
        input = replace(input, operator[1], operator[2])
    end
    instruction = "$output = $input"
    push!(instructions, parse(instruction))
end
close(f)

function execute(instructions, override=nothing)
    while !isempty(instructions)
        current = shift!(instructions)

        if !is(override, nothing) && (current.args[1] == :B)
            println("OVERRIDING B WITH $override")
            current = :(B = $override)
            override = nothing
        end

        try
            Core.eval(workspace, current)
            println(current)
        catch
    ...
    return Core.eval(workspace, :A)
end

A_PRIME = execute(copy(instructions))

# Needed to clear eval's "global" (module-level, in this case) scope.
module workspace end
println("BEGINNING SECOND PASS")

A = execute(copy(instructions), A_PRIME)
